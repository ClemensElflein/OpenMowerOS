...
# add_examples appends a single, idempotent OpenMowerOS configuration block to the given Raspberry Pi config.txt file.
# It ensures a [all] section exists, adds the labeled example block once, and returns without modifying
# the file if the file declares "DO NOT EDIT THIS FILE" or already contains the OpenMowerOS marker.
# Argument: path to the target config file.
